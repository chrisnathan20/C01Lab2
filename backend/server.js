...
// Delete a note belonging to the User given some noteId
...
app.patch("/editNote/:noteId", express.json(), async (req, res) => {
  try {
    ...
    if (!title && !content) {
      return res
        .status(400)
        .json({ error: "Either title or content is required." });
    }
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      // Construct updated parts of note
      const changes = {};
      ...
      await collection.updateOne({ _id: objectId }, { $set: changes });
